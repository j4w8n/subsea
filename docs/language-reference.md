# Subsea Language Reference

This is the detailed reference for subsea language syntax and semantics.
For installation and a "hello world" program, see [Getting Started](getting-started.md).

## Program Entry

Every program must define a top-level `main` function; subsea starts execution there.
...
- `main` is the process entry, not a called function, and cannot `ret`. Every reachable path in `main` must terminate with `linux.exit`, an equivalent terminating syscall, or an unconditional local jump to code that does not return.
...
See the [CLI Reference](cli.md), [Targets and Toolchains](targets.md), and
[Examples](examples.md) for compiler operation and complete build flows.
